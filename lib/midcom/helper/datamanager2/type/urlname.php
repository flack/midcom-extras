    ...
    public function _on_validate()
    {
        ...
        if (empty($this->value)) {
            if (   isset($this->_datamanager->types[$this->title_field])
                ...
                $copy->{$property} = midcom_helper_misc::urlize($this->_datamanager->types[$this->title_field]->value);
        ...
        if (!$resolver->name_is_safe($property)) {
            $this->validation_error = sprintf($this->_l10n->get('type urlname: name is not "URL-safe", try "%s"'), midcom_helper_misc::urlize($this->value));
            return false;
        }

        if (   !$this->allow_unclean
            && !$resolver->name_is_clean($property)) {
            $this->validation_error = sprintf($this->_l10n->get('type urlname: name is not "clean", try "%s"'), midcom_helper_misc::urlize($this->value));
            return false;
        }
